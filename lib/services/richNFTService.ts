...
export const getRichNFTInfoForAccount = async (accountId: string): Promise<EnrichedNFT[]> => {
  try {
    ...
    const enrichedNFTs = await Promise.all(
      allNFTs.map(async (nft: any): Promise<EnrichedNFT> => {
        try {
          ...
          console.warn(`Failed to enrich NFT ${nft.token_id}-${nft.serial_number}:`, (error as Error).message);
          return {
            tokenId: nft.token_id,
            serialNumber: nft.serial_number,
            owner: nft.account_id,
    ...
    return enrichedNFTs.filter(nft => nft !== null);
...
const resolveIPFS = (uri: string): string | null => {
  ...
  if (uri.startsWith('ipfs://')) {
    ...
    // Use ipfs.io - public gateway, no authentication needed
